...
void loadLabelNames(std::string labelNameFile, std::map<int, std::string> &labelNames)
{
	std::ifstream f(labelNameFile.c_str());
...
int main(int argc,char* argv[]){
    ...
    String modelTxt = "hb_mnist_deploy.prototxt";
    String modelBin = "hb_lenet_iter_10000.caffemodel";
    ...
    //! [Gather output] 获取概率值
    dnn::Blob prob = net.getBlob("hb_prob");   //[<Important>] gather output of "prob" layer
    int classId;
    double classProb;
    ...
    loadLabelNames("hb_label_name.txt", labelNames);

    //! [Print results] 输出结果
    std::cout << "Test file: " << imageFile << std::endl;
    std::cout << "Best class: #" << classId << "' " << labelNames[classId] << std::endl;
